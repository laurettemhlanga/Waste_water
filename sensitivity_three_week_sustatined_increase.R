...
Sensitivitydata_filter_sus <- rbindlist(lapply(seq_along(lead_ww),function(x)(ww_gold_standard_peak_sus
                                                                              ...
                                                                              %>% mutate(comparison_two  = ifelse(entered_lag == 1 & test_three_consecu == 1, "TP",
                                                                                                                  ifelse(entered_lag == 1 & test_three_consecu == 0, "FN",
                                                                                                                         ifelse(entered_lag == 0 & test_three_consecu == 1, "FP",
                                                                                                                                ifelse(entered_lag == 0 & test_three_consecu == 0, "TN", NA)))))
...
comparisondata_sus <- Sensitivitydata_filter_sus %>% 
  ...
  select(entered_lag, test_three_consecu,  comparison_two)# %>%
#summarise(n = n())


sensetivity_data_sus <- comparisondata_sus %>% 
  group_by(comparison_two, lead_ww, N_log_wks, wwthreshhold, percent) %>% 
  summarise(counts= n())  



sense_calulations_sus <- sensetivity_data_sus %>%
  # group_by(comparison_two, N_log_wks, percent, wwthreshhold, lead_ww) %>%
  pivot_wider(names_from = comparison_two,values_from = counts)%>%
...
ggsave("plots/sensitivity_sustained_three_week_increase.png", w = 14, h = 14)
