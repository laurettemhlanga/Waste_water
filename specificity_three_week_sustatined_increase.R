...
###############################################
# suststained increase three weeks 
# 
###############################################
...
#new data 
#new data 
ww_gold_standard_mod <- read.csv("data/new_ww.csv") %>% 
  mutate(in_trough = ifelse(use_for_spec == TRUE, 0, 
                            ifelse(use_for_spec == FALSE, 1, NA)) )

wastedata <- readxl::read_excel("data/wasterwater20220314.xlsx", 
...
ww_gold_standard_peak_sus <- rbindlist(lapply(1:nrow(indexes), 
                                              function(x)(wwgold_standardfiltered 
                                                          ...
                                                          %>% mutate(test_logcopies = ifelse(LoggenecopiesmL - n_weeks_logcopies_ave > log10(1+indexes[x,2]), 1 , # should it still be % increase
                                                                                             ifelse(LoggenecopiesmL - n_weeks_logcopies_ave < log10(1+indexes[x,2]), 0, NA)),
                                                                     test_three_consecu = ifelse(lag(test_logcopies, 2) == 1 & lag(test_logcopies,1) == 1 & test_logcopies == 1, 1, 
                                                                                                 ifelse(lag(test_logcopies, 2) == 1 & lag(test_logcopies,1) == 1 & test_logcopies == 0|
                                                                                                          lag(test_logcopies, 2) == 1 & lag(test_logcopies,1) == 0 & test_logcopies == 0|
                                                                                                          lag(test_logcopies, 2) == 0 & lag(test_logcopies,1) == 0 & test_logcopies == 0|
                                                                                                          lag(test_logcopies, 2) == 0 & lag(test_logcopies,1) == 0 & test_logcopies == 1|
                                                                                                          lag(test_logcopies, 2) == 0 & lag(test_logcopies,1) == 1 & test_logcopies == 1|
                                                                                                          lag(test_logcopies, 2) == 0 & lag(test_logcopies,1) == 1 & test_logcopies == 0|
                                                                                                          lag(test_logcopies, 2) == 1 & lag(test_logcopies,1) == 0 & test_logcopies == 1,
                                                                                                        0, NA)))
                                              )))
...
specificitydata_filter_sus <- rbindlist(lapply(seq_along(lead_ww),function(x)(ww_gold_standard_peak_sus
                                                                              ...
                                                                              %>% filter(in_trough_lag == 0)
                                                                              %>% drop_na(test_three_consecu) 
                                                                              %>% mutate(comparison_one  = ifelse(in_trough_lag == 1 & test_three_consecu == 1, "TP",
                                                                                                                  ifelse(in_trough_lag == 1 & test_three_consecu == 0, "FN",
                                                                                                                         ifelse(in_trough_lag == 0 & test_three_consecu == 1, "FP",
                                                                                                                                ifelse(in_trough_lag == 0 & test_three_consecu == 0, "TN", NA)))))
                                                                              
)))


comparisondata_sus <- specificitydata_filter_sus %>% 
  group_by( N_log_wks, wwthreshhold, lead_ww, percent) %>%
  select(in_trough_lag, test_three_consecu,  comparison_one)# %>%
#summarise(n = n())


specificity_data_sus <- comparisondata_sus %>% 
  group_by(comparison_one, lead_ww, N_log_wks, wwthreshhold, percent) %>% 
  summarise(counts= n())  



spe_calulations_sus <- specificity_data_sus %>%
  pivot_wider(names_from = comparison_one,values_from = counts)%>%
  mutate(specificity  = TN/(TN + FP),sample  = TN + FP)
...
spe_calulations_sus %>% 
  ggplot()+
  geom_point(aes(y = specificity, x = N_log_wks  , color = as.factor(percent)), size = 3)+
  ...
  geom_text(aes(x = N_log_wks, y = max(specificity, na.rm = T) + 0.2, label = sample), # some values are not the same 
            position = position_dodge(width = 0.25), size = 5)+
  facet_wrap(~lead_ww, labeller = labeller(lead_ww = facet_names))


ggsave("plots/specificity_sustained_three_week_increase.png", w = 14, h = 14)
