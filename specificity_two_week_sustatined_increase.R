...
specificitydata_filter_sus <- rbindlist(lapply(seq_along(lead_ww),function(x)(ww_gold_standard_peak_sus
                                                                              ...
                                                                              %>% mutate(comparison_one  = ifelse(in_trough_lag == 1 & test_two_consecu == 1, "TP",
                                                                                                                  ifelse(in_trough_lag == 1 & test_two_consecu == 0, "FN",
                                                                                                                         ifelse(in_trough_lag == 0 & test_two_consecu == 1, "FP",
                                                                                                                                ifelse(in_trough_lag == 0 & test_two_consecu == 0, "TN", NA)))))
...
comparisondata_sus <- specificitydata_filter_sus %>% 
  ...
  select(in_trough_lag, test_two_consecu,  comparison_one)


specificity_data_sus <- comparisondata_sus %>% 
  group_by(comparison_one, 
           lead_ww, N_log_wks, 
           wwthreshhold, percent) %>% 
  summarise(counts= n())  
...
spe_calulations_sus%>% 
  ...
  geom_text(aes(x = N_log_wks, y = max(specificity), label = sample), # some values are not the same 
            position = position_dodge(width = 0.25), size = 5)+
  facet_wrap(~lead_ww, labeller = labeller(lead_ww = facet_names))


ggsave("plots/specificity_sustained_two_week_increase_spec.png", w = 14, h = 14)
